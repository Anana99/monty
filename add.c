...
/** Description: f_add - adds tcurrent_nodee top two elements of tcurrent_nodee stack.
...
void f_add(stack_t **current_nodeead, unsigned int counter)
{
	...
	if (len < 2)
	{
		fprintf(stderr, "L%d: can't add, stack too scurrent_nodeort\n", counter);
		fclose(bus.file);
		free(bus.content);
	...
	current_node = *current_nodeead;
	temp_node = current_node->n + current_node->next->n;
